...
describe('M08 crop and precision acceptance in browser mode', () => {
  ...
  it('maps new precision and text gestures through an applied non-origin crop', async () => {
    ...
    await dragCanvas({ x: 30, y: 30 }, { x: 120, y: 100 }, 'm08-cropped-censor')
    await browser.waitUntil(
      async () =>
    ...
    expect(censor.transform.translateX).toBeCloseTo(90, 2)
    expect(censor.transform.translateY).toBeCloseTo(30, 2)
    expect(censor.localBounds.width).toBeCloseTo(90, 0)
    expect(censor.localBounds.height).toBeCloseTo(70, 0)

    await $('button[aria-label="Ruler"]').click()
    await dragCanvas({ x: 10, y: 200 }, { x: 210, y: 200 }, 'm08-cropped-ruler')
    await browser.waitUntil(
      async () =>
    ...
    expect(ruler.transform.translateX + rulerStart.x).toBeCloseTo(70, 0)
    expect(ruler.transform.translateY + rulerStart.y).toBeCloseTo(200, 0)
    expect(ruler.transform.translateX + rulerEnd.x).toBeCloseTo(270, 0)
    expect(ruler.transform.translateY + rulerEnd.y).toBeCloseTo(200, 0)
    ...
    await dragCanvas({ x: 150, y: 120 }, { x: 150, y: 120 }, 'm08-cropped-text')
    ...
    expect(createdText.transform.translateX).toBeCloseTo(210, 0)
    expect(createdText.transform.translateY).toBeCloseTo(120, 0)
  ...
  it('keeps the ruler frame and badge hittable through style growth and strong generic resize with undo/redo', async () => {
    ...
    await browser.waitUntil(async () => {
      const ruler = (await m08Snapshot()).document.layers.at(-1)
      return (
        Number(ruler?.transform.scaleX) < 0.15 &&
        Number(ruler?.transform.scaleY) < 0.4
      )
    })
