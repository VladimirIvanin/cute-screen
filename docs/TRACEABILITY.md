...
| REQ-CAP-008 | coordinate property fixtures           | mixed-DPI multi-monitor smoke       | planned |
| REQ-CAP-009 | backend selection integration          | X11 overlay + Wayland portal        | planned |
...
| REQ-ACT-003 | portal adapter integration             | GNOME/KDE binding flow              | planned |
| REQ-ACT-004 | fallback command test                  | copied command launches capture     | planned |
| REQ-ACT-005 | CLI snapshot + single-instance test    | installed binary invocation         | planned |
| REQ-ACT-006 | fake-platform E2E                      | hidden tray activation              | planned |
| REQ-ACT-007 | settings persistence                   | login/restart smoke                 | planned |
...
| REQ-EDT-008 | component ownership test               | toolbar visual snapshot             | planned |
| REQ-EDT-009 | layers panel component tests           | browser snapshot/interactions       | planned |
...
| REQ-LIB-002 | filmstrip visibility test              | empty/active series snapshots       | planned |
...
| REQ-UI-001  | token/component tests                  | 1600×1000 visual review             | planned |
| REQ-UI-002  | theme token tests                      | system/light/dark snapshots         | planned |
| REQ-UI-003  | locale completeness                    | live RU/EN switch E2E               | planned |
| REQ-UI-004  | responsive component tests             | 1024 px runtime review              | planned |
| REQ-UI-005  | semantic state tests                   | visual/contrast review              | planned |
| REQ-UI-006  | accessibility queries                  | keyboard/screen-reader audit        | planned |
| REQ-UI-007  | focus/reduced-motion tests             | keyboard-only E2E                   | planned |
| REQ-UI-008  | window configuration test              | native decoration smoke             | planned |
| REQ-UI-009  | diagnostics redaction tests            | exported bundle review              | planned |
| REQ-QLT-001 | 4K benchmark                           | reference runner report             | planned |
| REQ-QLT-002 | 8K benchmark                           | reference runner report             | planned |
| REQ-QLT-003 | scheduler instrumentation              | idle CPU/frame trace                | planned |
